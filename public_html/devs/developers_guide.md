...
## 1. Overview
The **Walk For God Bible App** is a specialized Single-Page Application (SPA) designed to run within a WordPress environment at the backend for authentication, memer management and payments processing. Its core philosophy is **"Offline-First, Personal Ownership"**. Unlike typical web apps that store user data in a central database, this Bible App empowers users to save their personal notes and edits directly to a **local CSV file** on their device using the modern **File System Access API**.
...
- **Data Storage**: 
    ...
    - **User Prefs**: Server-side secure CSV (`bible/users.db`).
...
| **users.db** | `/public_html/bible/users.db` | **User Preferences**. Stores device IDs or settings per user. |
...
| **options_api.php**| `/public_html/bible/options_api.php`| **API Endpoint**. Handles Read/Write operations for `users.db`. |
...
- **Format**: **Pick-Separated Values** (Char 254 Delimited).
